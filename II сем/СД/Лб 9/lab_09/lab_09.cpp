...
void Menu() {
    cout << "1. Добавить нового клиента\n"
        ...
        << "16. Найти клиента по индексу (интерполирующий поиск).\n"
        << "17. Найти клиента по фамилии (бинарный поиск).\n"
        << "18. Выход\n"
        << "Введите ваш выбор: ";
    cin >> choice;
};
...
int interpolationSearch(int n) {
    int left = 0;
    int right = clients.size() - 1;

    while (left <= right && n >= clients[left].index && n <= clients[right].index) {

        int pos = left + ((right - left) / (clients[right].index - clients[left].index)) * (n - clients[left].index);

        if (clients[pos].index == n) {
            cout << "Найден клиент по индексу " << n << ": " << clients[pos].name << endl;
            return pos;
        }
    ...
    cout << "Клиент с индексом " << n << " не найден.\n";
    return -1;
}
...
int binarySearch(const string& t) {
    ...
    while (left <= right) {
        ...
        int cmp = clients[mid].name.compare(t);

        if (cmp == 0) {
            cout << "Найден клиент с фамилией \"" << t << "\" под индексом " << mid << endl;
            return mid;
        }
    ...
    cout << "Клиент с фамилией \"" << t << "\" не найден.\n";
    return -1;
}
