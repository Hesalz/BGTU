...
void searchByAuthor(const string& filename, string authors) {
    ifstream inFile(filename);
    if (inFile.is_open()) {
        cout << "Результаты поиска книг по автору " << authors << ":" << endl;
        string author, title, publisher, section, originStr, availableStr;
        int origin, bookYear;
        bool available;
        bool found = false;
        ...
        if (!found) {
            cout << "Книги с указанным автором не найдены." << endl;
        }
        inFile.close();
    }
    else {
...
int main() {
    ...
    do {
        ...
        case 3:
            cout << "Введите автора: ";
            getline(cin, authors);
            searchByAuthor(filename, authors);
            break;
        case 0:
            break;
        default:
