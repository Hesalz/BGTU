...
{
	// Создание матрицы размером 3x3 для пересчета координат
	CMatrix M(3, 3);

	// Получение размера области в окне
	CSize sz = RW.Size();
	int dwx = sz.cx;
	int dwy = sz.cy;

	// Получение размера области в мировых координатах
	CSizeD szd = RS.SizeD();
	double dsx = szd.cx;    // Ширина в мск
	double dsy = szd.cy;    // Высота в мск

	// Расчет масштабов по x и y
	double kx = (double)dwx / dsx;   // Масштаб по x
	double ky = (double)dwy / dsy;   // Масштаб по y

	// Заполнение матрицы пересчета координат
	M(0, 0) = kx;  M(0, 1) = 0;    M(0, 2) = (double)RW.left - kx * RS.left;
	M(1, 0) = 0;   M(1, 1) = -ky;  M(1, 2) = (double)RW.bottom + ky * RS.bottom;
	M(2, 0) = 0;   M(2, 1) = 0;    M(2, 2) = 1;

	// Возвращение матрицы пересчета координат
	return M;
}
