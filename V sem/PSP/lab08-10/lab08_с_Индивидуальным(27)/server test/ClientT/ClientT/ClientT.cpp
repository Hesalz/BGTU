...
using namespace std;
using namespace chrono; // Для удобства добавить повторное подключение после Service Is Not Available
SOCKET  cC;
SOCKET  cS;
	...
	if (sel > 0)
	{
		...
		if (FD_ISSET(cC, &readSet))
		{
			...
			if (r > 0)
			{
				...
				else if (strstr(peekBuf, "ServiceNotAvailable") != NULL) {
...
int _tmain(int argc, char* argv[])
{
	...
	try
	{
		...
		while (true) {
			...
			if (strstr(message, "ServiceNotAvailable") != NULL) {
				printf("Service not available\n");
				puts("The requested service is not available at the current time according to server schedule.");
				closesocket(cC);
				WSACleanup();
				return -1;
			}
