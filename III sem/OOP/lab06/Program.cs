...
namespace lab05
{
    ...
    class Human : SentientBeing
    {
        ...
        public Human(string name) : base(name)
        {
            ...
            {
                throw new HumanNameEmptyException("Имя человека не может быть пустым.");
            }
        }
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
                try
                {
                    try
                    {
                        var humanEmpty = new Human("");
                    }
                    catch (HumanNameEmptyException ex)
                    {
                    ...
                    try
                    {
                        var armyEmpty = new Army();
                        armyEmpty.AddUnit(null);
                    }
                    catch (InvalidArmyOperationException ex)
                    {
                        Console.WriteLine($"Ошибка: {ex.Message}");
                    }
                    ...
                    try
                    {
                        var transformerEmpty = new Transformer("Бамблби", "Камаро", YearOfCreation.Year2010, new Specifications(1000, 1000));
                        transformerEmpty.CheckOverload(900);
                    }
                    catch (TransformerOverloadException ex)
                    {
                        Console.WriteLine($"Ошибка: {ex.Message}");
                    }
                    ...
                    {
                        Console.WriteLine($"Ошибка: Выход за пределы массива. {ex.Message}");
                    }

                    try { var array = new int[3]; Console.WriteLine(array[5]); }
                    catch (Exception) { throw; }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Произошло исключение: {ex.GetType()}: {ex.Message}");
                }
                finally
                {
            ...
            {
                Debug.Assert(b != 0, "Делитель не может быть нулем!");
                return a / b;
            }
