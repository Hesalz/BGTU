...
const App = () => {
  ...
  return (
    <div className=''>
      <header className='bg-white border-b-2 p-3 flex justify-between items-center gap-3 min-h-[67px]'>
          ...
          {products.map(product =>
            <li className='bg-white p-4 border-2 rounded flex flex-col items-center gap-2' key={product.id}>
              ...
              <div className='grid gap-2'>
                {product.quantity === 0
                  ? (
                    <button className='btn bg-neutral-800 text-white hover:bg-neutral-500' onClick={() => increaseQuantity(product.id)}>
                      Добавить в корзину
                    </button>
                  )
                  : (
                    <div className='grid gap-2'>
                      <button className='btn bg-red-200 hover:bg-red-300 border-2 border-red-400' onClick={() => removeFromCart(product.id)}>Удалить</button>
          ...
          <div className='p-3'>
            {cartProducts.length !== 0 ? (
              <div className='grid gap-4'>
                <ul className='grid gap-3'>
                  {cartProducts.map((product) =>
                    <li className='grid gap-2 place-items-center text-center' key={product.id}>
                      ...
                      <h3 className='font-bold text-sm'>{product.title}</h3>
                      <h3 className='font-bold'>Количество: {product.quantity}</h3>
                      <p>{formatter.format(product.price)}</p>
                ...
                <p className='text-center'><span className='font-bold'>Общая стоимость:</span> {formatter.format(cartProductDetail.price)}</p>
              </div>
            )
              : <p className='font-bold text-center'>Корзина пуста</p>
            }
          </div>
        </div>
      </div>
